		...
		while(!quit)
		{
			...
			else
			{
					...
					if(select==1) //책이름을 통해 장바구니 항목추가 책의 정보에는 이름 isbn 금액 3가지가 있음//
					{
						...
						switch(bname01)
						{
							case "두근두근 자료구조":
								...
								c[k].st=4.2;
								c[k].price=15000;
								System.out.println((k+1)+" Book Name:"+c[k].bname+"	\tISBN:"+c[k].ISBN+" \tAuthor:"+c[k].at+" \tRelease Date:"+c[k].date+" \tGenre:"+c[k].genre+" \tType:"+c[k].type+" \tStar:"+c[k].st+" \tPrice:"+c[k].price);
								k=k+1;
								break;
				...
				case 6:
					...
					{
						System.out.println((p)+" Book Name:"+c[p].bname+"\tISBN:"+c[p].ISBN+"\tPrice:"+c[p].price+"원");// 현재까지 추가된 항목 세부정보 띄우기//
					}
					System.out.println("");
				    ...
				    if (index >= 0 && index < k)
				    {
						...
						for(int p=0;p<k;p++)//0부터 클래스 배열 c의 끝인 k 번째 요소 까지 반복//
						{
							System.out.println((p+1)+" Book Name:"+c[p].bname+"	\tISBN:"+c[p].ISBN+" \tAuthor:"+c[p].at+" \tRelease Date:"+c[p].date+" \tGenre:"+c[p].genre+" \tType:"+c[p].type+" \tStar:"+c[p].st+" \tPrice:"+c[p].price);// 현재까지 추가된 항목 세부정보 띄우기//
						}
				    } 
				    else 
				    {
				...
				case 9:
					...
					{
						System.out.println((j+1)+". 책 이름:"+b[j].bookname+"\tISBN:"+b[j].isbn); //등록 요청된 책 리스트 보기//
					}
					break;
